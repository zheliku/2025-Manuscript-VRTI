...
\title{VRTI：为沉浸式\deleted{物理}学习提供真实触觉反馈与手部操作}
...
\added[id=zheliku]{
  本文提出虚拟-现实孪生交互（VRTI），通过手部重定向同步虚拟交互对象（VIO）与真实交互对象（RIO），为手势交互提供动态被动触觉反馈。该方法为手势操作提供被动力触觉的同时，通过基于手势预测的手部重定向实现虚实对齐，解决手势交互中的穿透问题。基于该方法实现三种VR孪生体（拉杆/按钮/旋钮），分别支持抓握、按压和捏取操作，应用于高中动量守恒沉浸式学习环境。用户研究表明，相比于GI（N=32），VRTI（N=32）：
